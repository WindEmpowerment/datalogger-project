...
unsigned char SD_init(void)
{
	...
	//Spi init
	spi_init();																/**envoyer l'initialition du périphérique SPI**/
	...
	for(i=0;i<10;i++){
		...
		SD_CS_DEASSERT;
		SPI_transmit (0xff);												/**transmission du 1er octet */
		SPI_transmit (0xff);												/**transmission du 2eme octet */
		...
		if(retry>0xfe)
		   {
			  newline(_COM1);
			  txString(_COM1,"\rinitialisation a échoué\n");
		...
		{
		case 1 : txString(_COM1,"\rver1.x\n");break;
		case 2 : txString(_COM1,"\rSDHC\n");break;
		case 3 : txString(_COM1,"\rver2.x\n");break;
		default :txString(_COM1,"\runknown Sd card\n");
		}
		//
...
unsigned char SD_readSingleBlock(unsigned long startBlock)
{
	...
	unsigned int i, retry=0;
		...
		retry = 0;
		while(SPI_receive() != 0xfe) 														/**attendre le start block qu'il prend la val 0xfe*/
		  if(retry++ > 0xfffe){
			  	 	 SD_CS_DEASSERT;
		...
		SPI_receive(); 																		/**laisser coulé 8 clock pulses*/
		SD_CS_DEASSERT;
...
unsigned char SD_writeSingleBlock(unsigned long startBlock)
{
	...
	unsigned int i, retry=0;
		...
		SPI_transmit(0xfe);     															/**envoyer le start block 0xfe (page 197 datasheet SD)*/
		...
		while(!SPI_receive()) 																/** attendre que la carte SD finis d'écrire et passe a l'état get idle*/
			if(retry++ > 0xfffe){SD_CS_DEASSERT; return 1;}									/**sortie de la fonction time-out*/

		SD_CS_DEASSERT;
		SPI_transmit(0xff);   																/**juste laisser passer 8 clock cycle delay avant la réaffirmation de CS line*/
